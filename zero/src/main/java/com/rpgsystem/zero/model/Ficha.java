...
public class Ficha { 
    ...
    // 2. REMOVIDO O 'final'. Variáveis final não funcionam bem com o Spring preenchendo formulários.
    ...
    // 3. CONSTRUTOR VAZIO PARA O SPRING BOOT
    // Colocamos os valores padrão aqui! Assim, quando o Spring criar a ficha 
    // a partir do formulário, ela já nasce com essas configurações.
    public Ficha() {
        this.nivel = 1;
        this.xpAtual = 0;
        this.xpProximo = 10;
    ...
    // Seu construtor original mantido (útil para quando você for criar via código no DAO)
    ...
    // 4. Transformado num método normal, já que a classe não é mais abstrata.
    public void exibirTipoFicha() {
        System.out.println("Ficha de Jogador Base");
    }
    ...
    // Adicionado o Setter para o usuarioId para o Spring conseguir injetar o valor
...
public String toString() {
    return """
            ...
            ===== STATUS DO HARDWARE (REALIDADE) =====
            Saldo Atual: R$ %.2f
            Custo de Vida Próximo: R$ %.2f (Vence em: %d dias)
            Nutrição Atual: %s
            Conexão: %s
            """
            .formatted(
                    nome, idade, nivel, xpAtual, xpProximo, classe, titulo,
                    forca, vida, agilidade, mana, carisma, sorte,
                    getDefesa(), getFlexibilidade(), getEsquiva(), getVontade(),
                    seduzir, intimidar, estudar, investigar, politica, religiao, artes, furtividade, construir,
                    saldoAtual, custoVida, diasRestantes, nutricao, conexao
            );
}
}